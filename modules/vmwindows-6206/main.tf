...
resource "azurerm_network_interface" "windows_nic" {
  ...
  name                = "${var.student_number}-win-nic-${count.index}"
  location            = var.location
  resource_group_name = var.resource_group_name
...
resource "azurerm_managed_disk" "windows_disk" {
  name                 = "${var.student_number}-win-disk"
  location             = var.location
  resource_group_name  = var.resource_group_name
...
resource "azurerm_windows_virtual_machine" "virtual_machine" {
  count               = var.windows_vm_count
  name                = "${var.student_number}-win-${count.index}"
  location            = var.location
  resource_group_name = var.resource_group_name
...
resource "null_resource" "windows_provisioner" {
  for_each = zipmap(
    [for idx, pip in azurerm_public_ip.public_ip : idx],
